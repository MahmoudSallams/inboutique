...
class ZooWXRParser
{
  ...
  function parse($file)
  {
    ...
    if (empty($file)) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'zoo-kodo'));
    }
    ...
    if (extension_loaded('simplexml')) {
      ...
      return new WP_Error('WXR_parse_error', esc_html__('No XML parser found.', 'zoo-kodo'));
    }
  }
  ...
  protected function simplexmlParse($file)
  {
    ...
    if (!$xml) {
      return new WP_Error('WXR_parse_error', esc_html__('Invalid WXR file.', 'zoo-kodo'), libxml_get_errors());
    }
  ...
  protected function xmlreaderParse($file)
  {
    ...
    $reader = new XMLReader();
		...
		if (!$status) {
			return new WP_Error('WXR_parse_error', esc_html__('Could not open the file for parsing', 'zoo-kodo'));
		}
