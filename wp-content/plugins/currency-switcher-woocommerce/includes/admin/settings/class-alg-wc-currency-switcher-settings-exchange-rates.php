...
class Alg_WC_Currency_Switcher_Settings_Exchange_Rates extends Alg_WC_Currency_Switcher_Settings_Section {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_'        . 'alg_get_exchange_rate', array( $this, 'get_exchange_rate_ajax' ) );
		add_action( 'wp_ajax_nopriv_' . 'alg_get_exchange_rate', array( $this, 'get_exchange_rate_ajax' ) );
	...
	function get_exchange_rate_ajax() {
		echo alg_get_exchange_rate( $_POST['alg_currency_from'], $_POST['alg_currency_to'] );
		die();
	}
	...
	function get_exchange_rates_settings( $settings ) {
		...
		$settings = array_merge( $settings, array(
			...
			array(
				...
				'type'      => 'select',
				'class'     => 'wc-enhanced-select',
				'options'   => alg_get_exchange_rates_servers(),
			),
		) );
				...
				$settings = array_merge( $settings, array(
					...
					array(
						...
						'options'   => array_replace(
							array(
								'default' => __( 'Default', 'currency-switcher-woocommerce' ) .
									' (' . alg_get_exchange_rates_server_title( get_option( 'alg_currency_switcher_exchange_rate_server', 'ecb' ) ) . ')',
							),
