...
if (!class_exists('ZooCustomizeManager')) :
    ...
    class ZooCustomizeManager
    {
        ...
        {
            $transport = 'refresh';
            $this->add_config( 'zoo-theme', array(
        ...
        function _ajaxReset()
        {
            if (!$this->wp_customize->is_preview()) {
                wp_send_json(array(
                    'success' => false,
                    'message' => esc_html__('Not previewing', 'zoo-theme')
                ));
            }
            if ( !check_ajax_referer('cust0miz3r-n0nc3', 'nonce', false)) {
                wp_send_json(array('success' => false, 'message' => esc_html__('Cheating?', 'zoo-theme')));
            }
            ...
            wp_send_json(array('success' => true, 'message' => esc_html__('Customize data reset successfully!', 'zoo-theme')));
        }
        /**
         * Import attachment
         */
        ...
        protected function sideLoadCustomizeImage($file)
        {
            if (!preg_match('/[^\?]+\.(jpe?g|jpe|gif|png)\b/i', $file, $matches)) {
                return new WP_Error('_invalid_attachment', esc_html__('Invalid attachment!', 'zoo-theme'));
            }
            $data = new \stdClass();
