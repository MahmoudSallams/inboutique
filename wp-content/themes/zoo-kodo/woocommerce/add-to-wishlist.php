...
<div class="yith-wcwl-add-to-wishlist  zoo-custom-wishlist-block add-to-wishlist-<?php echo esc_attr( $product_id ); ?>">
	<?php if( ! ( $disable_wishlist && ! is_user_logged_in() ) ): ?>
	    ...
	    <div class="yith-wcwl-wishlistaddedbrowse  zoo-custom-wishlist-btn hide" style="display:none;">
	        <a href="<?php echo esc_url( $wishlist_url )?>" rel="nofollow" data-toggle="tooltip" title="<?php echo apply_filters( 'yith-wcwl-browse-wishlist-label', $browse_wishlist_text )?>">
	            <i class="cs-font clever-icon-heart-1"></i> <span><?php echo apply_filters( 'yith-wcwl-browse-wishlist-label', $browse_wishlist_text )?></span>
	        </a>
	    </div>
	...
	<?php else: ?>
		<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_notice' => 'true', 'add_to_wishlist' => $product_id ), get_permalink( wc_get_page_id( 'myaccount' ) ) ) )?>" rel="nofollow" data-toggle="tooltip" title="<?php echo esc_attr($label); ?>" class="<?php echo str_replace( 'add_to_wishlist', '', $link_classes ) ?>" >
