 ...
 */
if (!function_exists('zoo_remove_assets_version'))
{
    function zoo_remove_assets_version($src)
    {
        if (false !== strpos($src, $_SERVER['SERVER_NAME'])) {
            $parts = explode('?', $src);
            return $parts[0];
        } else {
            return $src;
...
/**
 * Register Google fonts for Zoo Kodo
 *
 * @since Zoo Kodo 1.0
 *
