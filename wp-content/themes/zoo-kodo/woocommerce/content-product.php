...
?>
<li <?php post_class(); ?>>
    <div class="wrap-product-img images">
        <?php
        /**
            ...
            <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"
               data-resolution="<?php echo esc_attr( $resolution ) ?>" class="wrap-img">
                <img src="<?php echo esc_url($zoo_img_url); ?>"
                     height="<?php echo esc_attr( $zoo_height ) ?>" width="<?php echo esc_attr( $zoo_width ) ?>" class="lazy-img wp-post-image" data-original="<?php echo esc_attr( $zoo_img_url ) ?>"
        ...
        <?php if ( is_plugin_active('yith-woocommerce-compare/init.php') && !zoo_kodo_option( 'zoo_product_disable_compare_button' ) ) : ?>
            <div class="action-item action-compare" data-toggle="tooltip" data-placement="right" titlzoo_product_disable_wishlist_button="<?php echo esc_html__('Add to Compare', 'zoo-kodo'); ?>">
                <?php
                    $woocompare_action_add = 'yith-woocompare-add-product';
                    $woocompare_url_args = array(
                        'action' => $woocompare_action_add,
                        'id' => $product->get_id()
                    );

                    $lang = defined( 'ICL_LANGUAGE_CODE' ) ? ICL_LANGUAGE_CODE : false;
                    if( $lang ) {
                        $woocompare_url_args['lang'] = $lang;
                    }

                    $woocompare_add_product_url = apply_filters( 'yith_woocompare_add_product_url', esc_url_raw( add_query_arg( $woocompare_url_args, site_url() ) ), $woocompare_action_add );
                    $woocompare_is_button = get_option( 'yith_woocompare_is_button' );
                    $woocompare_button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Compare', 'zoo-kodo' ) );

                    echo '<div class="woocommerce product compare-button">';
                    printf( '<a href="%s" class="%s" data-product_id="%d" rel="nofollow">%s</a>', $woocompare_add_product_url, 'compare' . ( $woocompare_is_button == 'button' ? ' button' : '' ), $product->get_id(), $woocompare_button_text );
                    echo '</div>';
                ?>
            </div>
        <?php endif; ?>
