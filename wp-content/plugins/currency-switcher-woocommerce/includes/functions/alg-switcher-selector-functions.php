...
if ( ! function_exists( 'alg_get_currency_selector' ) ) {
	 ...
	 * @version 2.4.4
	 * @since   1.0.0
	 * @todo    (maybe) do not load `$currency_countries` and use empty string as default value instead
	 */
		...
		foreach ( $function_currencies as $currency_code ) {
			if ( isset( $currencies[ $currency_code ] ) ) {
				...
				if ( 'select' === $type ) {
					$data_icon = '';
					if ( $flags_enabled ) {
						$country_code = get_option( 'alg_wc_currency_switcher_flags_' . $currency_code, $currency_countries[ $currency_code ][0] );
	...
	function alg_currency_select_link_list() {
		...
		foreach ( $function_currencies as $currency_code ) {
			if ( isset( $currencies[ $currency_code ] ) ) {
				...
				$the_link = '<a id="alg_currency_' . $currency_code . '" href="' . add_query_arg( 'alg_currency', $currency_code ) . '">' . $flag_img .
					alg_format_currency_switcher( $currencies[ $currency_code ], $currency_code ) . '</a>';
