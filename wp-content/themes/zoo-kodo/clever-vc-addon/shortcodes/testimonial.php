...
<div id="<?php echo esc_attr($wrapID) ?>" class="cvca-testimonial-shortcode cvca-testimonial relative<?php echo esc_attr( $css_class ); ?> <?php echo esc_attr($atts['preset_style']) . ' ' . esc_attr($atts['style']) . '-testimonial' ?>">
            ...
            <?php while ( $the_query->have_posts() ) : $the_query->the_post(); ?>
                ...
                <?php if ( $atts['preset_style'] == 'default' ) : ?>
                    <article id="testimonial-<?php the_ID(); ?>" <?php echo post_class('cvca-testimonial-item ' . $class ) ?>>
                        <?php if ( get_post_meta( get_the_ID(), 'zoo_author_img', true ) != '' && !$atts['hide_avatar'] ) : ?>
                            <div class="cvca-wrap-avatar">
                                <img src="<?php echo wp_get_attachment_image_url( get_post_meta( get_the_ID(), 'zoo_author_img', true ), 'full' ) ?>" alt="<?php the_title(); ?>" class="avatar-circus" />
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="cvca-wrap-author-info">
                            <?php if ( get_post_meta( get_the_ID(), 'zoo_author', true) != '' ) : ?>
                                <h4 class="cvca-testimonial-author"><?php
                                    echo esc_html( get_post_meta( get_the_ID(), 'zoo_author', true ) ); ?>
                                </h4>
                            <?php endif; ?>

                            <?php if ( get_post_meta( get_the_ID(), 'zoo_author_des', true ) != '' ) : ?>
