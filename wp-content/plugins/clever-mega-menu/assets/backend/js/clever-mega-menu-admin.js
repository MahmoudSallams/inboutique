...
function clever_menu_close_vc_popup() {
    ...
    if (jQuery('.clever-mega-menu-customize-vc-iframe-wrapper', jQuery('body')).length) {
...
function clever_menu_upload_media(containerSelector, changeBtnI18n, wpMediaTitleI18n, wpMediaBtnTextI18n, multi)
{
    var frame,
        img = jQuery(containerSelector + ' .clever-mega-menu-upload-image'),
        input = jQuery(containerSelector + ' .clever-mega-menu-hidden-upload-image-value'),
        uploadBtn = jQuery(containerSelector + ' .clever-mega-menu-upload-btn');

    if (img.attr('src')) {
        uploadBtn.html('<span class="dashicons dashicons-admin-media"></span>'+changeBtnI18n);
    }
    ...
    uploadBtn.click(function(e)
    {
        ...
        frame = wp.media({
            title: wpMediaTitleI18n,
            button: {
                text: wpMediaBtnTextI18n
            },
            multiple: multi
        });
        ...
        frame.on('select',function()
        {
            ...
            img.attr('data-orientation', attachment.orientation);
            input.val(JSON.stringify(imgData));
        });
...
{
    // Color picker init.
    $('.clever-mega-menu-admin .color-picker').spectrum({preferredFormat: "rgb",showInput: true,showAlpha: true });

    clever_menu_insert_iconic_value('.clever-mega-menu-icon', '.clever-mega-menu-icons');

    // Upload media on settings screen.
    clever_menu_upload_media('#clever-mega-menu-logo-image', cleverMenuI18n.change, cleverMenuI18n.selectOrUpload, cleverMenuI18n.insert, false);
...
var cleverMenuAddEditBtn = function()
{
    ...
    jQuery('li', wpNavMenu.menuList).each(function()
    {
        ...
        if (jQuery('.clever-mega-menu-item-edit-btn', item).length === 0) {
            itemTitle.append('<span class="'+classAttributeValue+'"><i class="dashicons dashicons-admin-customizer"></i>'+cleverMenuI18n.editItem+'</span>');
