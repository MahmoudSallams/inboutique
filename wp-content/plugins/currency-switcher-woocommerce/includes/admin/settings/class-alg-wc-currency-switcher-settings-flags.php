...
class Alg_WC_Currency_Switcher_Settings_Flags extends Alg_WC_Currency_Switcher_Settings_Section {
	...
	function get_flags_settings( $settings ) {
		...
		$settings = array_merge( $settings, array(
			array(
				'title'     => __( 'Country Flags', 'currency-switcher-woocommerce' ),
				'type'      => 'title',
			...
			array(
				...
				'desc_tip'  => apply_filters( 'alg_wc_currency_switcher_plugin_option', sprintf(
					...
					'<a target="_blank" href="' . esc_url( 'https://wpcodefactory.com/item/currency-switcher-woocommerce-wordpress-plugin/' ) . '">' .
		...
		foreach ( alg_get_enabled_currencies( true ) as $i => $currency ) {
			if ( '' != $currency ) {
				$country_code    = $currency_countries[ $currency ][0];
				$settings = array_merge( $settings, array(
					array(
						...
						'options'  => array_merge( array( 'EU' => __( 'European Union', 'currency-switcher-woocommerce' ) ), alg_get_countries() ),
